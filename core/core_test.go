...
func TestBandmapMarker_OnFrequency(t *testing.T) {
	marker := BandmapMarker{Frequency: 3560000}
	entry := BandmapEntry{Frequency: 3560000}

	tt := []struct {
		desc      string
		frequency Frequency
	}{
		{desc: "exactly on the frequency", frequency: 3560000},
		{desc: "a bit above", frequency: 3560200},
		{desc: "a bit below", frequency: 3559800},
		{desc: "far above", frequency: 3562000},
		{desc: "far below", frequency: 3558000},
	}
	for _, tc := range tt {
		t.Run(tc.desc, func(t *testing.T) {
			assert.Equal(t, entry.OnFrequency(tc.frequency), marker.OnFrequency(tc.frequency),
				"a marker uses the same rule as a spot")
		})
	}
}
