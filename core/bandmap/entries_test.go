...
func TestEntry_Add_OnlySameCallAndSimilarFrequency(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.desc, func(t *testing.T) {
			...
			_, added := entry.Add(core.Spot{Call: core.MustParseCallsign(tc.call), Frequency: tc.frequency})
...
func TestEntries_AddNewEntry(t *testing.T) {
	...
	entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3760000, Time: now}, now, defaultWeights)
...
func TestEntries_QueryByFilters(t *testing.T) {
	now := time.Now()
	entries := NewEntries(&Notifier{}, countAllEntries)
	entries.Add(core.Spot{Call: core.MustParseCallsign("DL1ABC"), Frequency: 7020000, Band: core.Band40m, Source: core.ManualSpot}, now.Add(-1*time.Minute), defaultWeights)
	entries.Add(core.Spot{Call: core.MustParseCallsign("DL2ABC"), Frequency: 14015000, Band: core.Band20m, Source: core.ManualSpot}, now.Add(-2*time.Minute), defaultWeights)
	entries.Add(core.Spot{Call: core.MustParseCallsign("DL3ABC"), Frequency: 14030000, Band: core.Band20m, Source: core.ClusterSpot}, now.Add(-2*time.Minute), defaultWeights)
	entries.Add(core.Spot{Call: core.MustParseCallsign("DL4ABC"), Frequency: 28010000, Band: core.Band10m, Source: core.ClusterSpot}, now.Add(-2*time.Minute), defaultWeights)
...
func TestEntry_ReportedQualityWins(t *testing.T) {
	now := time.Now()
	entry := NewEntry(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m, Time: now})
	entry.applyReportedQuality()
	require.Equal(t, core.UnknownSpotQuality, entry.Quality)

	_, added := entry.Add(core.Spot{
		Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m,
		Time: now.Add(time.Minute), Source: core.SkimmerSpot, Quality: core.ValidSpotQuality,
	})

	require.True(t, added)
	assert.Equal(t, core.ValidSpotQuality, entry.Quality, "one spot with a reported quality is enough")
}

func TestEntry_ReportedQualitySurvivesAnUpdate(t *testing.T) {
	now := time.Now()
	entry := NewEntry(core.Spot{
		Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m,
		Time: now, Source: core.SkimmerSpot, Quality: core.ValidSpotQuality,
	})
	entry.applyReportedQuality()

	entry.update()

	assert.Equal(t, 1, entry.SpotCount)
	assert.Equal(t, core.ValidSpotQuality, entry.Quality, "the bandmap does not judge a reported quality")
}

func TestEntry_ReportedQualityOfTheSourceWithTheHighestPriority(t *testing.T) {
	now := time.Now()
	entry := NewEntry(core.Spot{
		Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m,
		Time: now, Source: core.RBNSpot, Quality: core.ValidSpotQuality,
	})
	entry.applyReportedQuality()

	entry.Add(core.Spot{
		Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m,
		Time: now.Add(time.Minute), Source: core.SkimmerSpot, Quality: core.BustedSpotQuality,
	})

	assert.Equal(t, core.BustedSpotQuality, entry.Quality, "the skimmer is nearer than the RBN")
}

func TestEntry_LatestReportedQualityOfTheSameSource(t *testing.T) {
	now := time.Now()
	entry := NewEntry(core.Spot{
		Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m,
		Time: now, Source: core.SkimmerSpot, Quality: core.BustedSpotQuality,
	})
	entry.applyReportedQuality()
	...
	assert.Equal(t, core.ValidSpotQuality, entry.Quality, "the source corrected itself")
}

func TestEntry_SNROfTheSourceWithTheHighestPriority(t *testing.T) {
	now := time.Now()
	entry := NewEntry(core.Spot{
		Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m,
		Time: now, Source: core.RBNSpot, SNR: 9,
	})
	entry.applyReportedSNR()
	require.Equal(t, 9.0, entry.SNR)

	entry.Add(core.Spot{
		Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m,
		Time: now.Add(time.Minute), Source: core.SkimmerSpot, SNR: 25,
	})

	assert.Equal(t, 25.0, entry.SNR, "the receiver of the nearest source decides")
}

func TestEntry_LatestSNROfTheSameSource(t *testing.T) {
	now := time.Now()
	entry := NewEntry(core.Spot{
		Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m,
		Time: now, Source: core.SkimmerSpot, SNR: 25,
	})
	entry.applyReportedSNR()

	entry.Add(core.Spot{
		Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m,
		Time: now.Add(time.Minute), Source: core.SkimmerSpot, SNR: 12,
	})

	assert.Equal(t, 12.0, entry.SNR, "the signal changed")
}

func TestEntry_WithoutAReportedSNR(t *testing.T) {
	now := time.Now()
	entry := NewEntry(core.Spot{
		Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m,
		Time: now, Source: core.ClusterSpot,
	})

	entry.applyReportedSNR()

	assert.Equal(t, 0.0, entry.SNR, "a cluster spot has no signal to noise ratio")
}

func TestEntries_Remove(t *testing.T) {
	now := time.Now()
	entries := NewEntries(&Notifier{}, countAllEntries)
	entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m, Time: now}, now, defaultWeights)
	entries.Add(core.Spot{Call: core.MustParseCallsign("dl2abc"), Frequency: 3540000, Band: core.Band80m, Time: now}, now, defaultWeights)

	removed := entries.Remove(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m})
...
func TestEntries_Remove_OnlyOnTheBandOfTheSpot(t *testing.T) {
	now := time.Now()
	entries := NewEntries(&Notifier{}, countAllEntries)
	entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m, Time: now}, now, defaultWeights)
	entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 14035000, Band: core.Band20m, Time: now}, now, defaultWeights)

	removed := entries.Remove(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m})

	assert.True(t, removed)
	require.Equal(t, 1, entries.Len())
	assert.Equal(t, core.Band20m, entries.entries[0].Band, "the same station runs on another band")
}

func TestEntries_Remove_OnAnotherFrequency(t *testing.T) {
	now := time.Now()
	entries := NewEntries(&Notifier{}, countAllEntries)
	entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m, Time: now}, now, defaultWeights)

	removed := entries.Remove(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3545000, Band: core.Band80m})

	assert.False(t, removed, "the entry is on another frequency")
	assert.Equal(t, 1, entries.Len())
}

func TestEntries_Remove_NotifiesTheListeners(t *testing.T) {
	now := time.Now()
	notifier := &Notifier{asyncRunner: func(f func()) { f() }}
	entries := NewEntries(notifier, countAllEntries)
	listener := new(entryRemovedSpy)
	notifier.Notify(listener)
	entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m, Time: now}, now, defaultWeights)

	entries.Remove(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Band: core.Band80m})

	require.Len(t, listener.entries, 1)
	assert.Equal(t, "DL1ABC", listener.entries[0].Call.String())
}

type entryRemovedSpy struct {
	entries []core.BandmapEntry
}

func (s *entryRemovedSpy) EntryRemoved(entry core.BandmapEntry) {
...
func TestEntries_CleanOutOldEntries(t *testing.T) {
	...
	entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Time: now.Add(-1 * time.Hour)}, now, defaultWeights)
	entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Time: now.Add(-30 * time.Minute)}, now, defaultWeights)
	entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Time: now.Add(-10 * time.Minute)}, now, defaultWeights)
	entries.Add(core.Spot{Call: core.MustParseCallsign("dl2abc"), Frequency: 3535500, Time: now.Add(-10 * time.Hour)}, now, defaultWeights)
	entries.Add(core.Spot{Source: core.WorkedSpot, Call: core.MustParseCallsign("dl3abc"), Frequency: 3535500, Time: now.Add(-50 * time.Hour)}, now, defaultWeights)
...
func TestEntries_Notify(t *testing.T) {
	...
	entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Time: now.Add(-40 * time.Minute)}, now, defaultWeights)
...
func TestEntry_Matches(t *testing.T) {
	now := time.Now()
	spot := core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3535000, Time: now.Add(-5 * time.Minute)}
	entry := NewEntry(spot)
	assert.Equal(t, core.UnknownSpotQuality, entry.Quality)

	similarSpot := core.Spot{Call: core.MustParseCallsign("dl2abc"), Frequency: 3535000, Time: now.Add(-2 * time.Minute)}
	...
	qsySpot := core.Spot{Call: core.MustParseCallsign("dl1abc"), Frequency: 3545000, Time: now.Add(-2 * time.Minute)}
	...
	bustedSpot := core.Spot{Call: core.MustParseCallsign("dl2abc"), Frequency: 3535000, Time: now.Add(-2 * time.Minute)}
	...
	completeDifferentSpot := core.Spot{Call: core.MustParseCallsign("dl3xyz"), Frequency: 3535000, Time: now.Add(-2 * time.Minute)}
...
func TestCalculateCallsignDistance(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.call1+" "+tc.call2, func(t *testing.T) {
			call1 := core.MustParseCallsign(tc.call1)
			call2 := core.MustParseCallsign(tc.call2)
...
func TestEntries_MarkAsWorked(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(string(tc.band)+" "+string(tc.mode), func(t *testing.T) {
			...
			entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Band: core.Band20m, Frequency: 14114000, Mode: core.ModeSSB, Time: now.Add(-11 * time.Minute)}, now, defaultWeights)
			entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Band: core.Band20m, Frequency: 14014000, Mode: core.ModeCW, Time: now.Add(-10 * time.Minute)}, now, defaultWeights)
			entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Band: core.Band40m, Frequency: 7073000, Time: now.Add(-30 * time.Minute)}, now, defaultWeights)
			entries.Add(core.Spot{Call: core.MustParseCallsign("dl2abc"), Band: core.Band80m, Frequency: 3535500, Time: now.Add(-12 * time.Hour)}, now, defaultWeights)
			entries.Add(core.Spot{Call: core.MustParseCallsign("dl1abc"), Band: core.Band80m, Frequency: 3535000, Time: now.Add(-1 * time.Hour)}, now, defaultWeights)

			entries.MarkAsWorked(core.MustParseCallsign("dl1abc"), tc.band, tc.mode)
