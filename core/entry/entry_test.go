...
func TestEntryController_Clear(t *testing.T) {
	...
	assert.Equal(t, "599", controller.myReport, "my report")
	assert.Equal(t, "001", controller.myNumber, "my number")
	assert.Equal(t, []string{"599", "001", ""}, controller.myExchange, "my exchange")
...
func setupEntryTest() (core.Clock, *mocked.Log, *mocked.QSOList, *mocked.EntryView, *Controller, *testSettings) {
	...
	view.On("SetSerialClaim", mock.Anything, mock.Anything, mock.Anything).Maybe()
...
func assertQSOInput(t *testing.T, qso core.QSO, controller *Controller) {
	...
	assert.Equal(t, qso.MyReport.String(), controller.myReport, "my report")
	assert.Equal(t, qso.MyNumber.String(), controller.myNumber, "my number")
	assert.Equal(t, qso.MyExchange, controller.myExchange, "my exchange")
