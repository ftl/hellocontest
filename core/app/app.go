...
func (c *Controller) ExportCabrillo() {
	...
	c.view.SelectSaveFile(c.exportCabrillo(result), "Export Cabrillo File", c.configuration.LogDirectory(), proposedName, "cabrillo", "cab")
}

func (c *Controller) exportCabrillo(result cabrillo.Result) func(filename string, err error) {
	return func(filename string, err error) {
		if err != nil {
			c.view.ShowErrorDialog("Cannot select a file: %v", err)
			return
